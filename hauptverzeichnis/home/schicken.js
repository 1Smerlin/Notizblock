function tableAuslesen() {
  ...
  let newlist = {}
  let table = document.getElementById('Verzeichnis')
  ...
  for (let i = 0; i < tr.length; i++) {
    ...
    for (let j = 0; j < td.length; j++) {
      if (td[j].getElementsByTagName('a').length === 1) {
        ...
        list(nummer, aText, ahref, klass, newlist)
        nummer += 1
...
function list(nummer, name, pfad, klass, newlist) {
  switch (nummer) {
    case 1:
      //naviliste 1
...
// Sende
function sendNewDiretory(ausgabe) {
  console.log('!!!---schick---!!!')
  console.log(ausgabe)
  url = window.location.protocol + "//" + window.location.hostname + ":" + (Number(window.location.port) + 1)
